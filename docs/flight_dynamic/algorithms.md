...
- **实现边界**：
  ...
  3. **步长上限**：六自由度机动（含地面滑跑/起落架等快动态）建议 `Step(dt)` 用 10-20 ms；
     100 ms 量级在起飞段发散（实测 roll 达 180° 量级后数值崩溃）。权威用法与回归见
     `fd_takeoff_substep_test`（dt=0.01 起飞稳定完成、dt=0.1 起飞段发散）；集成契约见
...
- **证据**：[evidence: tests/unit/flight_dynamic/fd_takeoff_substep_test.cpp]（起飞段 dt=0.01 完成、dt=0.1 发散）
