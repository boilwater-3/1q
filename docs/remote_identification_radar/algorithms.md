...
| 单目标 KF | `tracking/RirTrackFilter.cpp` | 量测 + 先验状态 → 预测/更新后验 | 6 维 CV 状态；动态 R 更新；LLT 失败跳过更新；q/std 下限 0.001 钳制（AR SignalComponentFactory 工厂同口径）；诚实初始化（2026-08-31 去真值速度种子：零速均值 + 速度无知先验 `initial_velocity_std_mps`（缺省 3000，暂无配置面——policy.tracking 未透出该键，待后续专场），位置/速度协方差分置——先验越大建轨期门越宽、收敛后自然收紧，关联门自适应来源；重捕后速度重新收敛） |
...
4. 战术决策、ECCM 决策与反欺骗（跟踪升级 N1-N7 已落地 LAPJV/池化/IMM）。
