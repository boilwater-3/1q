...
Authority: 阶段 3 common 化收敛的逐项评估结论（评估文档，非实施计划）。
...
为阶段 3 提供决策依据；**本文件不实施任何收敛**。

总结论：

1. **不立项全量 common 化**。收敛动机不是"复用"，而是**消除双源分叉风险**
   （副本 + 注明来源 commit 的形态下，任一侧修改都会制造隐性分叉）。
2. 按"物理恒等式 vs 装备私有口径"分类：**LAPJV 求解器、雷达方程全集、
   天线方向图四个模型**属物理恒等式，列入收敛清单；**检测单元账本、
   CFAR 判决链、传播/环境子集、航迹池**属装备私有或低收益，不动或缓。
3. 收敛执行采用 **"接触即收敛"（touch-then-consolidate）**：任何一侧因
   bug 修复或需求修改共享物理恒等式时，当次收敛到 `src/common/`，不修副本。
   缺省行为逐位一致（等价回归）是收敛门。不主动发起一次性大迁移。
...
- **接触即收敛**：任一侧修改 #1-#3 范围内的物理恒等式（bug/需求），
  当次将公共实现落到 `src/common/`，两侧适配层指向单源；本次修改的
  等价回归按 §2.1-3 执行。
- 阶段 3 其余既有条目不变：`max_range_m`/`recognition_dwell_sec` 四域
  归位是 RIR 内部配置语义小项（识别 policy → mission 域），与 common 化
  无关，单独立项执行。
- 本评估结论落入两侧模块 boundaries.md 引用即可，不新增守护脚本；
  分叉风险由"接触即收敛"流程约束，不做静态扫描。
