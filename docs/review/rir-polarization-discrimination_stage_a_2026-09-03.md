...
修订 4（2026-09-03，completeness-review 新证据）：Lane 1 审查发现两条中级数据流缺口，随本任务收口——
1、回放特征量测表（RirFeatureMeasurementRecordV1）缺 polarization_stats 子表，回放解码侧统计块静默丢失：fbs 加性补 RirPolarizationStatsObservationV1 子表（RIR3 内加性，不另升版）＋codec 双向映射＋往返断言。
2、回放事件级 payload_type 版本标签滞留 "RirCycleReplayRecordV2"（载荷已 RIR3）：RirRecordingSession/RirReplaySession 全部改 V3，事件级与 schema 级版本甄别对齐。
另登记低级备注：场景窗口裁剪方位差无 ±180° 缠绕处理（全角度字典下无影响，通用性备注）；Graves 对角捷径分支（|c|²≤1e-36）无测试触发（覆盖性备注）。
