...
透过率随观测几何变化，需按 §8 阶段 5 精细化（方向规划已于 2026-08-17 定稿：
几何路径量取大气壳层路径长度 d_atm，详见 §8 阶段 5；尚未实施）。
...
> **方向规划（2026-08-17）**：方向规划已定稿，待未来实施阶段落地；本次仅规划
> 并补充文档，未做任何代码变更。决策基线（用户确认）：
> - **几何路径量 = 大气壳层路径长度 d_atm**：LOS 与「地球半径 + 大气顶高度」球壳
>   求交截出的大气内路径长度（二次方程解析解）。不用 ECI 极坐标 el
>   （`asin(z/|los|)` 相对赤道面，非大气科学视线仰角，语义错位）；不做纯距离指数
>   衰减（不含角度信息）。
> - **d_max 反解**：以该目标当前 LOS 几何算出的 τ 为常数反解，不做路径自洽迭代
>   （文档注明近似语义：对远距离目标略乐观/悲观）。
> - **校验范围**：仅新增字段（路径尺度、大气顶高度），不动既有环境字段
>   （weather/sea/humidity/visibility/权重/base），沿用运行期 clamp 防御。
>
> 模型方向：`SbirsEnvironmentConfig` 新增 `atmospheric_path_reference_km`（参考路径
> 长度 d_ref，标定基准）、`atmospheric_shell_height_km`（大气顶高度）、几何修正开关
> （默认关闭 → 既有 235 例单测逐位不变）。每目标
> `d_atm = 球壳求交路径长度`，`τ_path = base^(d_atm/d_ref)`（Beer-Lambert 式指数长度
> 修正），`τ_eff = clamp(τ_path) × (1 − A_total)`；气象交互项框架（A_total 独立项/
> 交互项）保持不变，只在几何修正后的路径透过率上合成一次；波长维度保持单一波段，
> 不做 λ 连续积分（简易模型边界）。
>
> 语义边界：`base_atmospheric_transmittance` 从「总透过率标量」变为「参考路径长度
> d_ref 下的透过率」，指数外推做长度修正；开关默认关闭时语义不变。低轨场景物理
> 效果预期（验收基准）：地平线方向 d_atm ≫ 星下点方向 → τ 更小 → d_max 更小。
>
> 实现路径（未来实施顺序，本次不执行）：A. `SbirsGeometry` 新增
> `ComputeAtmosphericPathLengthM`（正切/穿过/未达大气三种情况）；B. 配置 + replay
> schema/codec + JSON 示例 + config_loader 全链路追加字段；C. 环境模型新增几何路径
> 透过率解析函数，保留 `ResolveWeatherAttenuation` 不变；D. 管线逐目标
> d_atm → τ_eff → SNR/d_max/归因，d_max 常数反解；E. 校验新 issue 码（路径尺度 ≥ 0
> 有限、大气顶高度 > 0 有限）；F. 测试（几何求交、环境模型 d_atm 变长 → τ 变小、
> 管线同目标不同 LOS 几何 → SNR/d_max 差异、replay roundtrip、默认关闭回归）；
> G. 文档同步（algorithms.md 气象小节/登记表、boundaries.md、data-flow.md、
> session_contract.md 如需、审计报告状态）。
>
> 收尾与验收：指标 3 判定从「✅ 满足（τ 为标量）」更新为「✅ 满足（τ 随几何路径）」；
> 契约测试固化六项指标；全量回归预期仅剩既有 `integration::airborne_radar`
> 0xc0000409 环境基线。
...
- 阶段 1/2 完成后，六项指标中 #1/#2/#4/#5 的合同判定应可翻绿；
  阶段 4 完成后 #6 翻绿；阶段 5 仅提升 #3 的物理保真度（方向规划已定，
  待实施阶段落地）。
