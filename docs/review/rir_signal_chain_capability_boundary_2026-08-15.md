...
Decisions: 2026-08-15 需求方确认——CFAR 采用统计级口径（AR 同款，§3.1）；
  四项增益采用"派生默认 + 四个 dB 偏置参数"（§3.2 方案 A）。
...
2. **七项代码存在于 AR 而未随迁**（审计 §4 判定为"AR 探测链本体"，刻意不迁）：
   ...
   **配置面已随迁**（`RirAntennaPatternConfig` + `enable_directional_pattern`），
   运行时从未评估。
3. **边界调和可行**：九项能力在 RIR 的正确形态是**识别驻留链路预算与回波有效性
   判定**，不是目标发现/点迹/航迹生产。据此 RIR 可全部具备九项能力而不破坏
   boundaries.md 非目标 #4（"不实现探测、关联、跟踪"）——本报告 §6 给出调和条款。
...
| 3 | 干扰功率计算 | 功率求解单源在 common（下列）；AR 持有单元聚合 `ArDetectionCellResolver.cpp:67-169`（目标单元时频重叠聚合、抗 RGPO 减半）与 `ArInterferenceObservationResolver`（J/N 门观测，AR 接收链语义） | ❌ 无；`RirReceiverConfig.interference_observation_jn_gate_db`（`RirHardwareConfig.h:87`）随迁未消费 | ✅ `1q/electromagnetics/RfScene.h:142-176` incident link 功率（`received_power_before_overlap_w`）、`TryEvaluateRfArrivalActivity`、`TryAggregateRfIncidentPower` |
...
3. **语义定位**：九项能力在 RIR 统一定位为"识别驻留链路预算与回波有效性判定"
   ——产出是**驻留观测质量**（特征维度有效性、观测退化因子），不是点迹、航迹
   或探测结论（非目标 #4 不破）。
...
| 9 | 恒虚警检测 | **迁（驻留检测门语义）** | RIR 落地 `RirDwellDetectionGate`：Pfa 门限 + Swerling Pd + 确定性种子判决（副本改写 `RadarEquations` 检测子集 + `SignalDetector` 判决链），输出"驻留回波有效性"驱动特征维度门控升级（现状硬编码 SNR<6 dB → 检测门/可配置回退）。**不产点迹航迹、不做关联跟踪**；CA-CFAR 口径按 §3.1 澄清后再议 |
| 10 | 驻留指向（前置） | **阶段 2a 先行**（审计 §5.1 既定评估项转正式） | `PrepareLrrPointing` 优先航迹选择逻辑迁入 RIR 自管波束调度；提供波束指向 → 离轴角 → 方向图增益链路。RIR 仍**不驱动外部雷达波束**（不新增模块间波束服务接口） |
| 11 | 环境输入面（前置） | **阶段 2a 扩展** | `RirCycleInput` 增加环境快照（传播损耗、杂波功率）与 RF 入射链路（incident links）；调用方编排与航迹供给同模式（无代码依赖）。校验/issue codes/replay 兼容性随迁扩展 |

**判定为"不做"的相邻能力**（显式否决，防蔓延）：点迹生成与距离/多普勒量测提取
（`ArDetectionCellResult.echo_delay_s`/`two_way_doppler_shift_hz` 语义）、量测误差
模型（`MeasurementErrorModel`）、抗 RGPO 前沿跟踪减半（ECCM 语义属 AR）、J/N 门
干扰观测输出。若后续需求要求，须重开边界评审。

## 6. 与现有非目标的调和

boundaries.md 非目标 #4（"不实现探测、关联、跟踪；航迹由外部雷达供给"）与需求
九项能力的调和条款：

1. **"探测"的语义切分**：非目标 #4 否决的是**目标发现**（点迹→检测→航迹的探测
   链产品语义）；九项能力落地的是**驻留回波物理化评估**（链路预算语义）。RIR
   消费外部航迹的架构不变，检测门输出只影响自身特征维度有效性，不对外发布
   "目标发现"事件。
2. boundaries.md 中"本模块无探测链"（F1/F2 节）的表述按此精化：**RIR 无探测链
   （不产点迹/航迹），但具备识别驻留链路预算**（方向图/分项 SINR 账本/驻留检测
   门，阶段 2 落地，见本报告 §7）。
3. 非目标 #4 增补限定："不把驻留检测门的判决解释为自身探测结果"。

## 7. 分阶段落地建议（已展开为
`docs/review/remote_identification_radar_phase2_plan_2026-08-15.md` §2-§4）

> 时序约束（关键）：等价性测试逐字段锁定 SNR 口径；§3.3 的口径升级必然破坏该
> 锁定。落地顺序必须与《阶段 1 计划》§9 的阶段 2 主线（删除 AR 侧识别耦合 +
> 等价性测试退役）衔接：**先删 AR 侧、退役等价性测试，再升级 SNR 口径**；若需
> 并行，则以配置开关保持旧口径直通（默认旧行为，开关显式启用物理化账本）。

- **阶段 2a（输入面与前置）**：决策 10/11——驻留指向迁移（自管波束调度）、
  `RirCycleInput` 环境快照 + incident links、传播损耗接线；校验/issue/replay
  同步；新增输入面单测与跨模块供给契约测试。
- **阶段 2b（驻留链路预算）**：决策 1/5/6/7/8/9——`RirAntennaPatternRuntime`
  （4 模型 + 旁瓣/后瓣/扫描损失单测）、`RirDwellLinkBudget`（分项 SINR 账本 +
  §3.2 四增益偏置参数及其校验/issue codes）、`RirDwellDetectionGate`（统计级
  CFAR 口径，Pfa 统计验证：蒙特卡洛虚警率 ≈ 配置 Pfa）；特征维度门控升级为
  检测门驱动（保留 6 dB 口径可配置回退）；algorithms.md 登记表与
  boundaries.md F1/F2 节同步。
- **阶段 3（common 化，计划 R2 既定扩展）**：`RirRadarEquations` 副本与 AR
  `RadarEquations` 本体收敛至 `src/common/`；方向图评估与检测单元求解的 common
  化同批评估；AR `PropagationModel`/杂波源 common 化评估（决策 7 尾项）。
