...
2. **观测工具面**：Replay 落盘、信封派生投影（debug view / lifecycle recorder /
...
- 可观测性单源（信封唯一结构化事实、投影只读信封、Replay 唯一落盘、`PROJECT_LOG` 人读旁路；产品形态齐套表）
...
- `session_contract.md` —— 有 Session 的传感器模块的统一会话契约（会话配置直接赋值、Session 组合所有权、运行期配置提交、电源单源、两通道+可选投影输出、可观测性单源、Replay 持久化语义）。
