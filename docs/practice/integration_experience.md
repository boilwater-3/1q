...
职责（维持现状）。**本文件是分析结论，行动项尚未实施**；实施后须更新本文件与
对应模块的设计文档。
...
- **根因**：recorder 需显式 `Attach` 接线、头文件淹没在 session 目录、集成者从
  ...
  指引注释。
- **建议**：
  1. 示例改用 LifecycleRecorder（消除自研逻辑与 bug，示范库设施正确用法）；
  2. Session 头文件注释补充"集成侧事件通知首选：Attach 对应 LifecycleRecorder"。

### F2 平台运动学 → ECEF 与方位/距离投影缺位（工具缺位，2 处重复）
...
- `MakeEnuBasis` / `MakeTargetStates`（方位/距离 → ECEF 偏移）：两示例目标脚本
  各写一遍同一套 ENU 基投影。
- **建议**：`coordinate` 域（纯函数共享域）补两个单函数：
  `TryMakeEcefVelocityFromHeading(heading_deg, speed_mps, origin_lla, *out)` 与
  bearing/range → ECEF offset。目标**脚本编排**仍是业务层（冻结契约 §7），仅上移
  几何单函数。

### F3 FlightManager 集成契约文档缺失（文档缺失，试错成本最高）
...
- **建议**：`FlightManager.h` / `Maneuver.h` 注释补充：kTakeoff 的 latitude_rad
  语义、建议步长（含滑跑/起落架 ≤ 10-20 ms 的说明）、PushManeuver 的状态迁移。
  纯注释级修复，不改 API。
...
| 优先级 | 行动 | 涉及面 | 性质 |
|---|---|---|---|
| P0 | 示例改用 LifecycleRecorder，消除自研事件判定（F1） | examples/component_attachment + Session 头注释 | 边界修正 |
| P0 | FlightManager.h / Maneuver.h 补 kTakeoff 航向、步长、状态机注释（F3） | include/1q/flight_dynamic | 文档 |
| P1 | coordinate 补平台运动学 → ECEF 与方位/距离投影单函数，两示例改用（F2） | include/1q/coordinate + 2 示例 | 工具缺位 |
| P2 | ArCycleResult 内部帧字段注释（F5 附带）；EOS 校验提示（F4） | 头文件注释 | 文档 |
| 维持 | Session 输入样板 / 场景编排 / 配置调参 / 事件总线选型 | — | 有意职责 |

若行动项涉及跨模块 API 变更或契约争议，按 `docs/common/open_questions.md` 流程
登记后再实施；P0 两项为示例侧改动与注释补充，可直接实施。

## 变更规则

- 实施任一行动项后：更新本文件对应条目（标记已实施/日期/涉及 commit），并同步
  相关模块设计文档（boundaries.md 若涉及边界声明）。
