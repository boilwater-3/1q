# FlatBuffers 构建期代码生成配置
#
# 将 schemas/replay/*.fbs 在构建期编译为 *_generated.h，取代手工提交的
# checked-in 生成头。flatc 编译器来自 conan flatbuffers 包（FlatcTargets.cmake
# 暴露的 flatbuffers::flatc imported executable 以及 FLATBUFFERS_FLATC_EXECUTABLE
# 变量），版本与运行时头一致（当前 1.12.0）。
#
# 本文件只定义函数；实际的 flatc 发现与 schema 编译由 src/CMakeLists.txt
# 在 find_package(flatbuffers) 之后调用，因为 flatbuffers::flatc target 直到
# ProjectDependencies.cmake 执行后才存在。
...
# 发现并校验 flatc。必须在 find_package(flatbuffers) 之后调用。
#
# 优先级：
#   1. conan 暴露的 flatbuffers::flatc imported target（FlatcTargets.cmake）
#   2. FLATBUFFERS_FLATC_EXECUTABLE 变量（同一来源的另一形态）
#   3. find_program(flatc) fallback（PATH，仅用于脱离 conan 的本地诊断）
macro(oneq_setup_flatc)
...
# 为单个 .fbs schema 创建生成规则。
#
#   oneq_flatbuffers_generate(<schema_file> <sensor_subdir> <out_var>)
#
# <sensor_subdir> 是 src/ 下的 sensor 目录名（如 sar、electro_optical_sensor），
# 用于在输出目录中复刻 <sensor>/session/generated/ 层级，保证 Style-2 include
# （`#include "<sensor>/session/generated/<name>_generated.h"`）可解析。
#
# 结果通过 <out_var> 返回生成头的绝对路径，调用方将其加入 target 的 SOURCES
# 以建立编译依赖。
#
# 生成参数与原 checked-in 头一致：--cpp --gen-object-api（Object API 内容
# 虽未被手写 codec 消费，但保留以维持与历史生成头字节级一致，避免无谓 diff）。
function(oneq_flatbuffers_generate schema_file sensor_subdir out_var)
