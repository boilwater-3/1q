...
common::TargetFeatureList BuildBatchTargets(std::size_t count,
                                            float x_bias,
                                            float y_bias,
                                            float z_bias) {
  ...
  for (std::size_t i = 0; i < count; ++i) {
    targets.push_back(BuildTarget(
        300000u + static_cast<std::uint64_t>(i), 120.0f + static_cast<float>(i % 5),
        0.0f, 0.0f, 0.9f + static_cast<float>(i % 3) * 0.1f,
        x_bias + static_cast<float>(i) * 8.0f,
        y_bias + static_cast<float>(i % 9) * 1.5f,
        z_bias + static_cast<float>(i % 4) * 1.0f));
  }
  return targets;
}
...
std::unordered_set<std::uint64_t> BuildAssociationKeySet(
    const common::TrackOutputFrame& frame) {
    ...
    case 1U:
      return environment_test::MemorySceneBuilder()
          .AddJammer(BuildJammerEmitter(
              environment::JammingTechnique::kNoiseSuppression, 10.0f, 8.0f,
              0.20f, 0.10f, true))
          .BuildSceneState();
    case 2U:
      return environment_test::MemorySceneBuilder()
...
CycleStats RunCycleAndCaptureStats(
...
TEST(RadarDetectionStressTest, LargeBatchSingleCyclePublishesStableTrackOutput) {
  ...
  for (std::size_t i = 0; i < target_tiers.size(); ++i) {
    ...
    const common::TargetFeatureList targets =
        BuildBatchTargets(target_count, 500.0f, 10.0f, 3.0f);

    common::TrackOutputFrame frame;
    const CycleStats stats = RunCycleAndCaptureStats(
        &controller, &radar_context, &signal_pipeline, targets, 0U, &frame);
...
TEST(RadarDetectionStressTest,
     LargeBatchMultiCyclePreservesOutputSetAcrossDuration) {
  ...
  for (std::size_t i = 0; i < target_tiers.size(); ++i) {
    ...
    common::TargetFeatureList targets =
        BuildBatchTargets(target_count, 800.0f, 16.0f, 4.0f);
    ...
    for (std::size_t cycle = 0; cycle < 10U; ++cycle) {
      ...
      common::TrackOutputFrame frame;
      stats.push_back(RunCycleAndCaptureStats(
          &controller, &radar_context, &signal_pipeline, targets,
...
TEST(RadarDetectionStressTest,
     LargeBatchSceneSwitchingMaintainsReadableOutput) {
  ...
  common::TargetFeatureList targets =
      BuildBatchTargets(target_count, 700.0f, 10.0f, 2.0f);
  std::vector<CycleStats> stats;
  stats.reserve(12U);
...
TEST(RadarDetectionStressTest,
     LongDurationPatrolKeepsMetricsWithinReasonableBounds) {
  ...
  common::TargetFeatureList targets =
      BuildBatchTargets(target_count, 900.0f, 12.0f, 3.0f);
  std::vector<CycleStats> stats;
  ...
  for (std::size_t cycle = 0; cycle < cycle_count; ++cycle) {
    ...
    const std::size_t previous_command_count =
        radar_context.SubmittedCommands().size();
    CycleStats cycle_stats = RunCycleAndCaptureStats(
        &controller, &radar_context, &signal_pipeline, targets,
        previous_command_count, nullptr);
...
TEST(RadarDetectionStressTest, LoadStaircaseRemainsStableAcrossScaleTransitions) {
  ...
  for (std::size_t tier_index = 0; tier_index < target_tiers.size();
    ...
    common::TargetFeatureList targets = BuildBatchTargets(
        target_tiers[tier_index], 600.0f + static_cast<float>(tier_index) * 20.0f,
        8.0f, 2.0f);
