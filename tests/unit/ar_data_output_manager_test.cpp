...
model::TrackStateSnapshot BuildTrackSnapshot(std::uint64_t association_key,
                                                 model::TrackStatus status) {
  model::TrackStateSnapshot snapshot;
  snapshot.velocity_x = 120.0f;
  snapshot.velocity_y = 0.0f;
  snapshot.velocity_z = 0.0f;
  snapshot.speed = 120.0f;
  snapshot.rcs = 3.0f;
  snapshot.association_key = association_key;
...
TEST(DataOutputManagerTest, BuildsTrackOutputFrameWithCountsAndLostFlag) {
  ...
  model::TrackStateSnapshotList track_snapshots;
  track_snapshots.push_back(BuildTrackSnapshot(101U, model::TrackStatus::kConfirmed));
  track_snapshots.push_back(BuildTrackSnapshot(202U, model::TrackStatus::kLost));
  ...
  EXPECT_TRUE(output::CountTracksByStatus(frame, model::TrackStatus::kLost) > 0U);
}

TEST(DataOutputManagerTest, BuildsDecisionInputFrameFromTrackOutputFrame) {
  signal::assembly::DataOutputManager output_manager;
  model::TrackStateSnapshotList track_snapshots;
  track_snapshots.push_back(BuildTrackSnapshot(303U, model::TrackStatus::kConfirmed));
  ...
  EXPECT_EQ(frame.tracks[0].association_key, 303U);
  EXPECT_EQ(frame.tracks[0].status, model::TrackStatus::kConfirmed);
}

TEST(DataOutputManagerTest, EmptyTrackOutputFrameKeepsZeroCounts) {
  signal::assembly::DataOutputManager output_manager;
  const output::TrackOutputFrame frame =
      output_manager.BuildTrackOutputFrame(5U, 12U, model::TrackStateSnapshotList());
