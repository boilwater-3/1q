...
using config::RirVegetationScatterPhysicsConfig;
using internal::RirEnvironmentSceneState;
using internal::RirPropagationModel;

RirEnvironmentSceneState MakeScene(RirVegetationCoverProfile profile, bool enable_physical) {
  RirEnvironmentSceneState scene;
  scene.vegetation_scatter_physics.cover_profile = profile;
  scene.vegetation_scatter_physics.enable_physical_model = enable_physical;
  return scene;
}

/// @brief 植被物理关闭：传播损耗 = 4+1.5+1 = 6.5 dB，杂波 = 基线 3 dB。
TEST(RirPropagationModelTest, DisabledVegetation_UsesBaselines) {
  const RirPropagationModel model;
  const auto result = model.Evaluate(MakeScene(RirVegetationCoverProfile::kDisabled, false));
...
TEST(RirPropagationModelTest, EnabledVegetation_RaisesClutterAboveBaseline) {
  ...
  for (auto profile : profiles) {
    const auto result = model.Evaluate(MakeScene(profile, true));
...
TEST(RirPropagationModelTest, DeterministicAcrossCalls) {
  const RirPropagationModel model;
  const auto scene = MakeScene(RirVegetationCoverProfile::kDeciduousForest, true);
...
TEST(RirPropagationModelTest, DenseProfileExceedsGrassland) {
  const RirPropagationModel model;
  const auto grass = model.Evaluate(MakeScene(RirVegetationCoverProfile::kOpenGrassland, true));
  const auto dense = model.Evaluate(MakeScene(RirVegetationCoverProfile::kTropicalDense, true));
