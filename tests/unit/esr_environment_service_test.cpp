...
TEST(EsrEnvironmentServiceTest, UnknownTechniqueWithPositiveRiskInfersMixedAndKeepsBothChannels) {
  ...
  jammer.technique = session::EsrJammingTechnique::kUnknown;
  jammer.active = true;
  ...
  const session::EsrEnvironmentSnapshot snapshot = service.SampleEnvironment();

  ASSERT_EQ(snapshot.jammer_sources.size(), 1U);
  EXPECT_EQ(snapshot.jammer_sources.front().technique, session::EsrJammingTechnique::kMixed);
...
TEST(EsrEnvironmentServiceTest, UnknownTechniqueWithZeroRiskInfersSuppressionOnly) {
  ...
  ASSERT_EQ(snapshot.jammer_sources.size(), 1U);
  EXPECT_EQ(snapshot.jammer_sources.front().technique, session::EsrJammingTechnique::kNoiseSuppression);
...
TEST(EsrEnvironmentServiceTest, DeceptionOnlySourceDoesNotTriggerSuppressionDetection) {
  ...
  context.dt_sec = 1.0f;
  session::EsrJammerSource jammer;
  jammer.technique = session::EsrJammingTechnique::kDeception;
  jammer.active = true;
  jammer.center_hz = 10.0e9;
  jammer.bandwidth_hz = 2.0e9;
...
TEST(EsrEnvironmentServiceTest, AtmosphericPhysicsCanIncreasePropagationLoss) {
  ...
  const session::EsrEnvironmentSnapshot baseline_snapshot = service.SampleEnvironment();

  session::EsrEnvironmentCycleContext physics_context = baseline_context;
  ...
  const session::EsrEnvironmentSnapshot physics_snapshot = service.SampleEnvironment();
...
TEST(EsrEnvironmentServiceTest, ConfigAtmosphericPhysicsAppliesWhenSceneDoesNotOverride) {
  ...
  context.dt_sec = 1.0f;
  context.observation.propagation_profile = session::EsrPropagationEnvironmentProfile::kOpen;
...
TEST(EsrEnvironmentServiceTest, AtmosphericContextCanChangePropagationLoss) {
  config::EsrEnvironmentScenarioConfig baseline_scenario;
  ...
  config::EsrEnvironmentScenarioConfig stressed_scenario = baseline_scenario;
