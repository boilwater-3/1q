...
bool IsRatioValid(float value) {
  return IsFinite(value) && value >= 0.0f && value <= 1.0f;
}

void ValidatePlatformPose(const oneq::common::PoseState& platform_pose, EosValidationIssueList* issues) {
  if (issues == nullptr) {
    return;
  }
...
void ValidateTarget(const EosTargetState& target, std::size_t target_index,
  ...
  if (target.target_id == 0U) {
    issues->push_back(MakeIssue(EosValidationSeverity::kWarning, EosValidationCode::kInvalidTargetId,
                                target_index, "target id is zero"));
  }

  if (!IsFinite(target.range_m) || !IsFinite(target.azimuth_deg) || !IsFinite(target.elevation_deg) ||
      !IsFinite(target.apparent_temperature_k) || !IsFinite(target.emissivity) ||
      !IsFinite(target.reflectance) || !IsFinite(target.projected_area_m2)) {
  ...
  if (target.range_m <= 0.0f) {
    issues->push_back(MakeIssue(EosValidationSeverity::kError, EosValidationCode::kInvalidTargetRange,
                                target_index, "target range must be positive"));
  }
...
EosValidationIssueList ValidateEosCycleInput(const EosCycleInput& input) {
  ...
  if (!IsFinite(input.solar_irradiance_w_m2) || input.solar_irradiance_w_m2 < 0.0f) {
    issues.push_back(MakeIssue(EosValidationSeverity::kError, EosValidationCode::kInvalidSolarIrradiance,
                               static_cast<std::size_t>(-1),
                               "solar irradiance must be finite and non-negative"));
  }
  if (!IsRatioValid(input.atmospheric_transmittance)) {
    issues.push_back(MakeIssue(EosValidationSeverity::kError,
                               EosValidationCode::kInvalidAtmosphericTransmittance,
                               static_cast<std::size_t>(-1),
                               "atmospheric transmittance must be in [0, 1]"));
  }
