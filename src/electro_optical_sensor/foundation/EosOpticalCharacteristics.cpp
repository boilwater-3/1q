...
float ComputeSlantRangeByDepressionAngle(float platform_altitude_m, float depression_deg) {
  const float safe_altitude_m = oneq::internal::numerics::SafePositive(platform_altitude_m, 1.0f);
...
float ComputeFocalHeightRatio(float focal_length_m, float platform_altitude_m) {
  return oneq::internal::numerics::SafePositive(focal_length_m, 0.1f) / oneq::internal::numerics::SafePositive(platform_altitude_m, 1.0f);
}

float ComputeInstantaneousFovDeg(float detector_size_m, float focal_length_m) {
  const float safe_detector_size_m = oneq::internal::numerics::SafePositive(detector_size_m, 1.0e-3f);
  const float safe_focal_length_m = oneq::internal::numerics::SafePositive(focal_length_m, 0.1f);
...
float ComputeDiffractionLimitedAngularResolutionRad(float wavelength_um, float aperture_diameter_m) {
  ...
  const float safe_aperture_m = oneq::internal::numerics::SafePositive(aperture_diameter_m, 0.1f);
...
float ComputeDefocusCoefficient(float focus_distance_m, float target_distance_m) {
  const float safe_focus_distance_m = oneq::internal::numerics::SafePositive(focus_distance_m, 1.0f);
  const float safe_target_distance_m = oneq::internal::numerics::SafePositive(target_distance_m, 1.0f);
...
float ComputeCircleOfConfusionDiameterM(float aperture_diameter_m, float focal_length_m,
  ...
  return safe_aperture_m * defocus * safe_focal_length_m / oneq::internal::numerics::SafePositive(focus_distance_m, 1.0f);
}
