...
// 噪声分量统计（design 2.8 ComputeBackgroundNoiseStatistics）。
struct SbirsNoiseStatistics {
  double photon_noise_w{0.0};    // 光子（散粒）噪声 RMS（W）
  double thermal_noise_w{0.0};   // 热噪声 RMS（W）
  double readout_noise_w{0.0};   // 读出噪声 RMS（W）
  double total_noise_w{0.0};     // 三项 RMS 合成
};

// 计算背景噪声统计。背景辐射经孔径与积分时间转换为光子噪声；热噪声由探测器温度
// 与带宽估算；读出噪声取硬件 readout_noise_rms_w。三者均方根合成得 total_noise_w。
SbirsNoiseStatistics ComputeBackgroundNoiseStatistics(const config::SbirsHardwareConfig& hardware);

// 由噪声统计与硬件 NEP 取大，得到有效噪声（W）。
// 当背景/热/读出参数为默认 0 时，total_noise_w 为 0，回退到 NEP。
