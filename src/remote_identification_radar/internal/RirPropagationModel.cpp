 ...
 * @brief RIR 环境层传播/杂波模型实现（副本改写自 PropagationModel.cpp；
 *        阶段 2-M M5）。
 */
...
config::RirVegetationScatterPhysicsConfig ResolveVegetationScatterConfig(
...
VegetationPhysicsParameters ResolveVegetationPhysicsParameters(
...
float ComputeVegetationClutterMultiplier(
  ...
  for (std::size_t i = 0U; i < x_param.size(); ++i) {
...
RirPropagationResult RirPropagationModel::Evaluate(
  ...
  const config::RirVegetationScatterPhysicsConfig vegetation_config =
