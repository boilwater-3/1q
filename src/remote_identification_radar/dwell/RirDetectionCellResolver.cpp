...
bool TryResolveCellInterference(
    const RirDetectionCellConfig& config, double echo_delay_s, double target_cell_center_hz,
  ...
  for (const auto& ordered_link : ordered) {
    ...
    const long double effective_power =
        static_cast<long double>(link.received_power_before_overlap_w);
...
bool AreGainsUsable(const config::hardware::RirSignalProcessingConfig& gains) {
...
bool TryResolveRirDetectionCell(
  ...
  RirDetectionCellResult candidate;
