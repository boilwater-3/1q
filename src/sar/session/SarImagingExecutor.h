...
// 执行 L1 RDA 聚焦成像。当 policy.enable_l2_motion_compensation 为真时，先在 raw history
// 上执行一阶运动补偿（基于理想/实际轨迹缓冲），再进入 RDA。成功时写入：
...
// 行为与诊断字符串与历史实现逐字符保持一致，以维持 replay 严格相等。
...
// 执行 L3 BP 聚焦成像，逐脉冲实际轨迹来自 actual_trajectory_buffer。成功时写入：
//   - result.focused_image（按 retain_focused_image 决定全矩阵或占位元数据）
