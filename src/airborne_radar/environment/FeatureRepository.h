...
class FeatureRepository final : public IFeatureRepository {
 public:
   ...
   */
  bool QueryBestMatch(const FeatureVector& input, MatchResult& result) const override;
 ...
 private:
   ...
   */
  static std::vector<FeatureRecord> TransformRowsToRecords(const std::vector<RawFeatureRow>& rows);
   ...
   */
  static float ComputeDistance(const FeatureVector& input, const FeatureRecord& record);
