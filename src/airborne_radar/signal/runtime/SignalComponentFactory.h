...
struct LifecycleAssemblyArtifacts {
  ...
  std::vector<std::unique_ptr<tracking::KalmanUpdater>>
...
class SignalComponentFactory final {
 ...
 private:
   ...
   * @return 已创建的更新器。
   */
  static std::unique_ptr<tracking::KalmanUpdater> CreateKalmanUpdater(float measurement_noise_std) {
    tracking::KalmanUpdaterConfig updater_config;
    updater_config.measurement_noise_std = std::max(measurement_noise_std, 0.001f);
    return std::unique_ptr<tracking::KalmanUpdater>(new tracking::KalmanUpdater(updater_config));
  }
  /**
   * @brief 构建 IMM 转移矩阵。
   * @param config 顶层配置。
