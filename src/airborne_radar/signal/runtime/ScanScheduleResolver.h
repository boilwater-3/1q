 ...
 */
inline std::vector<common::AzimuthElevationDeg> BuildScheduledScanPattern(
    const common::AzimuthElevationLimitsDeg& limits, float az_step_deg, float el_step_deg,
  ...
  if (!finite_limits || limits.az_min_deg > limits.az_max_deg || limits.el_min_deg > limits.el_max_deg ||
      ...
      el_step_deg <= 0.0f) {
    return std::vector<common::AzimuthElevationDeg>();
  }
  ...
  std::vector<common::AzimuthElevationDeg> pattern;
  ...
  if (sequence == oneq::common::ScanSequence::kAzimuthFirst) {
    for (std::size_t el_index = 0; el_index < el_values.size(); ++el_index) {
      ...
      for (std::size_t az_order = 0; az_order < az_values.size(); ++az_order) {
        ...
        common::AzimuthElevationDeg pointing;
 ...
 */
inline common::AzimuthElevationDeg ResolveScheduledBeamPointing(
    const common::RadarOrientationConfig& orientation_config,
    const detection::EffectiveBeamwidthDeg& effective_beamwidth_deg, std::uint32_t cycle_index) {
  common::AzimuthElevationLimitsDeg effective_limits = common::IntersectScanLimits(
...
inline void ApplyScanScheduleToRuntimeConfig(std::uint32_t cycle_index,
  ...
  common::RadarOrientationConfig& orientation = runtime_config->beam_control.radar_orientation;
