...
struct ResolvedBeamState {
  ...
  common::model::AzimuthElevationDeg beam_pointing_deg; /**< 挂架坐标系下的当前波束中心。 */
...
class BeamControlResolver {
 public:
   ...
   * @return 当前探测使用的波束状态。
   */
  static ResolvedBeamState Resolve(const signal::config::AntennaConfig& antenna_config,
 ...
 private:
   ...
   */
  static common::model::AzimuthElevationDeg ResolveMountFrameBeamPointing(
      const common::model::RadarOrientationConfig& orientation_config,
      const common::model::PlatformAttitudeDeg& platform_attitude_deg) {
    const common::model::AzimuthElevationLimitsDeg effective_limits =
    ...
    if (orientation_config.stabilization_mode ==
        common::model::StabilizationMode::kBodyStabilized) {
      return common::utils::ComputeMountFrameBeamPointing(orientation_config);
    }

    common::model::AzimuthElevationDeg desired_platform_pointing_deg;
    ...
    common::model::AzimuthElevationDeg stabilized_mount_frame_pointing;
