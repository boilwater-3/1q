...
class DataAssociationEngine {
 public:
   ...
   */
  AssociationResult AssociateDetections(const model::TargetFeatureList& targets,
   ...
   */
  std::vector<std::uint64_t> Associate(const model::TargetFeatureList& targets,
                                       const std::vector<std::uint8_t>& detection_succeeded);
  /**
 ...
 private:
   ...
   */
  Eigen::Vector3f BuildPositionVector(const model::TargetFeature& target) const;
  /**
   * @brief 判断目标是否带有可用的笛卡尔位置量测。
   * @param target 输入目标特征。
   * @return 若存在位置量测则返回 true。
   */
  bool HasPositionMeasurement(const model::TargetFeature& target) const;
  /**
   * @brief 从位置量测初始化高斯状态。
   * @param position 位置量测。
   * @return 初始化后的高斯状态。
