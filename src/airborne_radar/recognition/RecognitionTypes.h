 ...
 *
 * 识别链路内部类型（`src/airborne_radar/recognition/`），不进入 public API。
 * 观测为效能级派生：由目标真值特征列表经 SNR、距离、带宽、驻留与视角覆盖
 * 等效能约束转换而来，保证可解释、可配置、可回放。
 */

#ifndef AIRBORNE_RADAR_RECOGNITION_RECOGNITION_TYPES_H_
#define AIRBORNE_RADAR_RECOGNITION_RECOGNITION_TYPES_H_
...
/**
 * @brief RecognitionObservationContext 单周期识别观测上下文（效能约束）。
 */
struct RecognitionObservationContext {
...
/**
 * @brief RcsObservation RCS 特征观测（dBsm 域）。
 */
struct RcsObservation {
...
/**
 * @brief MotionObservation 运动特征观测（由滤波航迹多周期估计派生）。
 */
...
/**
 * @brief PolarizationObservation 双通道极化特征观测（dB 域）。
 */
struct PolarizationObservation {
...
/**
 * @brief RangeProfileObservation 宽带一维距离像特征观测。
 */
struct RangeProfileObservation {
...
/**
 * @brief RecognitionFeatureSet 单周期四维特征观测集合。
 */
struct RecognitionFeatureSet {
  RcsObservation rcs{};
  MotionObservation motion{};
  PolarizationObservation polarization{};
  RangeProfileObservation range_profile{};
  /** @brief 本周期有效特征维度掩码（ArRecognitionFeatureDimension 按位或）。 */
...
#endif  // AIRBORNE_RADAR_RECOGNITION_RECOGNITION_TYPES_H_
