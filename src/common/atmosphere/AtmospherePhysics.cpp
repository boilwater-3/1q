...
float blake_atmos_loss_r4_1(float h_a_m, float f_hz, float theta_deg, float r_m, float k) {
  return static_cast<float>(blake_atmos_loss_r8_1(static_cast<double>(h_a_m),
                                                   static_cast<double>(f_hz),
                                                   static_cast<double>(theta_deg),
                                                   static_cast<double>(r_m),
...
double blake_atmos_loss_r8_1(double h_a_m, double f_hz, double theta_deg, double r_m, double k) {
  ...
  const double pressure_ratio = EstimatePressureFromAltitudeHpa(safe_altitude_m) / kSeaLevelPressureHpa;
  ...
  const double specific_attenuation_db_per_km = oxygen_specific_db_per_km + vapor_specific_db_per_km;
...
Gtd7Profile GTD7(int day_of_year, double sec, double alt_m, double glat_deg, double glong_deg,
  ...
  const double temperature_k = std::max(kMinKelvin, base_temperature_k * solar_factor * seasonal_factor);
...
AtmosphericPropagationResult EvaluateAtmosphericPropagation(
  ...
  const double mid_altitude_m =
      0.5 * (static_cast<double>(inputs.radar_altitude_m) + static_cast<double>(inputs.target_altitude_m));
  ...
  const double dry_pressure_hpa = pressure_hpa * (1.0 - 0.15 * Clamp(inputs.relative_humidity, 0.0f, 1.0f));
  const double temperature_k = inputs.temperature_k > 0.0f ? static_cast<double>(inputs.temperature_k)
                                                            : profile.temperature_k;
  const double temperature_c = temperature_k - 273.15;

  const double blake_loss_db = blake_atmos_loss_r8_1(mid_altitude_m, inputs.frequency_hz,
                                                      inputs.elevation_deg, inputs.path_length_m,
                                                      inputs.k_factor);
  ...
  const double n_index_h =
      refractivity_index_nh_r4(static_cast<float>(n_index), static_cast<float>(mid_altitude_m),
  ...
  const double density_factor =
      Clamp(profile.density_kg_m3 / kSeaLevelDensity, 0.05, 4.0);
