...
struct JsonInternalParser {
      ...
      if (c == '\\') {
        ...
        switch (esc) {
          ...
          case 'u': {
            ...
            if (end != hex + 4) { *error = "invalid \\uXXXX escape"; return false; }
            if (cp <= 0x7F) {
  ...
  static bool ParseArray(std::istream& in, JsonValue* out, std::string* error) {
    ...
    for (;;) {
      ...
      if (!Value(in, &out->children_.back(), error)) return false;
  ...
  static bool ParseObject(std::istream& in, JsonValue* out, std::string* error) {
  ...
  static bool Value(std::istream& in, JsonValue* out, std::string* error) {
    ...
    if (c == '{') { in.get(); return ParseObject(in, out, error); }
    if (c == '[') { in.get(); return ParseArray(in, out, error); }
