...
float ToDb(double ratio) {
  return static_cast<float>(
      10.0 * std::log10(std::max(ratio, oneq::common::numerics::kNumericFloor)));
}
...
std::uint32_t ResolveAvailablePulseCount(const CycleTimingBaseParams& params) {
  ...
  const double pulse_window =
      static_cast<double>(params.cycle_dt_sec) /
      std::max(params.pri_s, oneq::common::numerics::kNumericFloor);
...
float ComputeDynamicThresholdSnrDb(double noise_power_w, const StatisticalDetectionParams& params) {
  const double safe_noise_power_w = std::max(noise_power_w, oneq::common::numerics::kNumericFloor);
  ...
  threshold_factor =
      std::max(threshold_factor, oneq::common::numerics::kNumericFloor) * threshold_scale;
