...
/** @brief 内部环境执行态配置（与 EsrEnvironmentScenarioConfig 字段兼容）。 */
using EnvironmentConfig = environment::EsrEnvironmentScenarioConfig;
...
inline extension::InterceptPipelineConfig BuildPipelineConfig(const EsrInternalExecutionConfig& internal) {
  ...
  ext.detection.min_detect_snr_db = internal.detection.min_detect_snr_db;
  ...
  ext.statistical_detection.min_snr_db = internal.detection.min_detect_snr_db;
