...
class ScanPatternGenerator final {
 public:
  ...
  static std::vector<BeamPointingDeg> Generate(const ScanPatternConfig& config) {
    ...
    const bool start_from_right =
        (config.start_pos == 1 || config.start_pos == 2);
    const bool start_from_bottom =
        (config.start_pos == 2 || config.start_pos == 3);
    if (start_from_right) {
    ...
    if (config.sequence == 0) {
      for (std::size_t el_index = 0; el_index < el_values.size(); ++el_index) {
        const bool reverse_row =
            config.enable_serpentine && ((el_index % 2U) == 1U);
        for (std::size_t az_order = 0; az_order < az_values.size(); ++az_order) {
          const std::size_t az_index =
              reverse_row ? (az_values.size() - 1U - az_order) : az_order;
    ...
    for (std::size_t az_index = 0; az_index < az_values.size(); ++az_index) {
      const bool reverse_column =
          config.enable_serpentine && ((az_index % 2U) == 1U);
      for (std::size_t el_order = 0; el_order < el_values.size(); ++el_order) {
        const std::size_t el_index = reverse_column
                                         ? (el_values.size() - 1U - el_order)
                                         : el_order;
        BeamPointingDeg beam;
        beam.az_deg = az_values[az_index];
        beam.el_deg = el_values[el_index];
 ...
 private:
   ...
   * @return 方位采样序列。
   */
  static std::vector<float> BuildAzimuthSequence(const ScanPatternConfig& config,
                                                 float az_step) {
   ...
   * @return 俯仰采样序列。
   */
  static std::vector<float> BuildElevationSequence(
      const ScanPatternConfig& config, float el_step) {
    std::vector<float> el_values;
    const float el_min = std::max(-90.0f, std::min(config.start_el_deg,
                                                    config.end_el_deg));
    const float el_max = std::min(90.0f, std::max(config.start_el_deg,
                                                   config.end_el_deg));
