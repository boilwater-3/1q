...
聚焦算法编号必须与 `SAR_MODULE_DESIGN.md` 的 `FocusingAlgorithm` 保持一致：`0=Auto, 1=RDA, 2=CSA, 3=OmegaK, 4=GBP, 5=BP`。当自动选择逻辑改变用户指定算法时，总控模块应在日志中输出结构化警告，记录请求算法、实际算法、降级原因和场景规模。
...
| CSA/Omega-K | 暂缓 | 进入 Phase 3 前需单独补算法公式、适用边界和参考结果 |
| 自动选择 | 暂缓 | 只有当 RDA/GBP/BP 均有回归测试、性能基准和相位一致性验证后才能启用 |
| 相位重参考 | 条件通过 | 仅用于全局相位基准对齐，不得替代运动补偿、RCMC 或自聚焦 |
| 辐射定标 | 暂缓 | Phase 4 前需补发射功率、天线增益、标定目标、RCS 量纲和噪声模型闭环 |
