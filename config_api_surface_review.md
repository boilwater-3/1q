...
| `AtmosphericDerivedContext.solar_flux_f107a/f107` | ⚠️ **保留但 optional 语义未闭环** | 空间天气数据，外部事实。当前 AR 仍为裸 `float`，尚无 `has_`/optional 语义，文档问题单未关闭 |
| `AtmosphericDerivedContext.geomagnetic_ap` | ⚠️ **保留但 optional 语义未闭环** | 同上 |
...
| `EnvironmentDefaultConfig.jamming_detection_threshold_db` | ✅ **已移除（2026-04-18）** | 对外仅保留 `JammingSensitivityProfile` 语义档位，阈值由库内映射 |
...
| 大气高级上下文（solar/ap）无 optional 语义 | `AtmosphericDerivedContext.solar_flux_f107a/f107/geomagnetic_ap` 增加 `has_` 标志或改为 `std::optional`（C++17）/ 哨兵值模式；该项当前仍为 open question |
...
| `EsrAtmosphericPhysicsConfig.k_factor` | ✅ 已具备 optional 语义 | 已由 `SpaceWeatherContext.has_k_factor` 承接（默认值可用） |
| `EsrAtmosphericPhysicsConfig.day_of_year` | ✅ 已具备 optional 语义 | 已由 `SpaceWeatherContext.has_day_of_year` 承接（默认值可用） |
...
> [!SUCCESS]
> 该项已收口：AR 公开 API 已删除 `jamming_detection_threshold_db`，仅保留 `JammingSensitivityProfile`。
...
| **P1** | AR `jamming_detection_threshold_db` 兼容字段去留决策（保留 or 删除） | ✅ 已完成：已删除兼容字段（2026-04-18） |
