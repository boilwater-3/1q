/**
 * @file baseline_takeoff_east/main.cpp
 * @brief 场景 baseline_takeoff_east 可执行薄入口：场景 JSON 由编译宏钉死，共用 RunScene。
 *
 * 调试参数 --cycles/--view-every/--output-dir（见 app/runner.h）；本目录
 * baseline_takeoff_east.json 为场景描述、baseline_takeoff_east.md 为期望表。任意场景通用 runner：
