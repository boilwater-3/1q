/**
 * @file fleet_area_division_circle/main.cpp
 * @brief 场景 fleet_area_division_circle 可执行薄入口：场景 JSON 由编译宏钉死，共用 RunScene。
 *
 * 调试参数 --cycles/--view-every/--output-dir（见 app/runner.h）；本目录
 * fleet_area_division_circle.json 为场景描述、fleet_area_division_circle.md 为期望表。任意场景通用 runner：
