/**
 * @file scenes/scene_script.h
 * @brief 自定义实体-组件示例：世界模型目标真值脚本。
 *
 * 消费方世界模型：场景文件（scenes/*.json，见 scenes/scene_data.h）的目标脚本
...
struct TargetEcefState {
  ...
  std::vector<remote_identification_radar::session::RirPolSMatrixSample> rir_pol_dictionary{};
  /**< 极化散射字典（全量，开机一次性载入；架构 B） */
