...
| 宿主 | 两卫星实体 + 地面站 `FusionComponent`（内挂 `FusionEngine` 与 `PrecisionEvaluationSession`） |
...
| 卫星ECEF三维导航定位误差 | 120 | 真值ECEF/错误三维ECEF/误差向量/\|误差\|（2026-08-23 复核：三维行已随错误三维落地输出；配方姿态抽样为 0°，误差 0.0m） | 能写（2026-08-22 表记 `无` 已过时） |
