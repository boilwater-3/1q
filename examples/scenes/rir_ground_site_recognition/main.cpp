/**
 * @file rir_ground_site_recognition/main.cpp
 * @brief 场景 rir_ground_site_recognition 可执行薄入口：场景 JSON 由编译宏钉死，共用 RunScene。
 *
 * 调试参数 --cycles/--view-every/--output-dir（见 app/runner.h）；本目录
 * rir_ground_site_recognition.json 为场景描述、rir_ground_site_recognition.md 为期望表。任意场景通用 runner：
