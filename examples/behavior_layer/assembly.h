 ...
 *
 * 装配层职责（冻结契约 §5）：
 *   - 创建平台实体（长机 + 僚机）并挂载数据组件；
 *   - 会话与引擎（ArSession / FusionEngine / AreaCoveragePlanner）放入
 *     registry ctx 的 BehaviorContext，生命周期由装配层管理；
 ...
 * @note 会话与引擎生命周期由装配层管理；世界目标事实由消费方脚本每周期注入
 *       （去真值化纪律下目标事实属消费方场景编排）。
 */
struct BehaviorContext {
  ...
  std::vector<airborne_radar::session::ArTargetInput> world_targets{}; /**< 世界目标事实（消费方注入） */
  airborne_radar::session::ArCycleResult last_ar_result{}; /**< 最近一次 AR 周期结果（供消费方读取） */
};

/**
 * @brief 装配行为层：创建长机 + 僚机实体、注入上下文、挂载组件。
 * @param[in] registry 目标 registry。
 * @param[in] config AR 会话配置（复用 examples/configs 同源 JSON）。
