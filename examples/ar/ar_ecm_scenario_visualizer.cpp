  ...
  auto session = std::unique_ptr<airborne_radar::session::RadarSession>(
      new airborne_radar::session::RadarSession(airborne_radar::session::RadarSessionFactory::Create(
          airborne_radar::config::RadarExpertSessionConfigBuilder(
              ...
              .End()
              .Environment()
              .WithJammingSensitivityProfile(environment::ResolveJammingSensitivityProfile(5.0f))
              .End()
              .Build()));
...
airborne_radar::environment::EnvironmentSceneState BuildScene(int cycle) {
  ...
  if (cycle >= kPhaseDeception && cycle < kPhaseRecovery) {
    ...
    deception_jammer.has_direction_deg = true;
    deception_jammer.azimuth_deg = -5.0f;
    deception_jammer.elevation_deg = 0.5f;
    deception_jammer.angular_span_deg = 4.0f;
  ...
  if (cycle >= kPhaseRepeater && cycle < kPhaseRecovery) {
    ...
    repeater_jammer.has_direction_deg = true;
    repeater_jammer.azimuth_deg = 3.0f;
    repeater_jammer.elevation_deg = 0.0f;
    repeater_jammer.angular_span_deg = 6.0f;
