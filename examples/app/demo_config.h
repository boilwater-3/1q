...
// （examples/log/，见 CMakeLists.txt 的 CA_DEFAULT_OUTPUT_DIR）；
// 未注入时的回退为相对路径（随运行目录）。
...
/// 加载六份会话配置（复用各域 config_loader 与 examples/basic_config/ 同源 JSON）。
...
/// 之后应用，数据源为场景文件（scenes/scene_data.h），替代原 demo_config 内硬编码
...
#endif  // EXAMPLES_APP_DEMO_CONFIG_H_
