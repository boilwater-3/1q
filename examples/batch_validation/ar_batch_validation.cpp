 ...
 *
 * @par 运行方式
 *   ./ar_batch_validation [output_dir]
...
constexpr const char* kScenarioHeader =
    "scenario_id,target_range_km,rcs_m2,target_count,min_snr_db_override,"
...
ScenarioSummary RunArScenario(const ArCase& c, const ar_config::ArSessionConfig& base_config,
  ...
  // 录制 scope：TraceSession + 所有 Step 包在内，结尾 Flush。
  {
    ...
    for (std::uint32_t i = 0; i < kNumCycles; ++i) {
      ...
      std::fprintf(cycle_writer.file(),
                   "%s,%u,%d,%d,%d,%zu,%zu,%zu,%zu,%zu,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,"
                   "%.5f,%zu,%zu,%zu,%zu\n",
  ...
  } else if (replay.playback.compared_output_count != s.executed_cycles) {
    s.warnings.Warn("replay compared_count=" + std::to_string(replay.playback.compared_output_count) +
                    " != executed=" + std::to_string(s.executed_cycles));
  }
...
int main(int argc, char** argv) {
  ...
  // 3. 构造场景并逐个执行。
  std::vector<ArCase> cases = BuildArCases();
  ...
  for (std::size_t i = 0; i < cases.size(); ++i) {
    ...
    summaries.push_back(RunArScenario(cases[i], base_config, output_dir, cycle_writer));
    cycle_writer.Flush();
  }
