/**
 * @file ArRecognitionResult.h
 * @brief 远程目标识别结果与周期摘要类型。
 *
 * 识别结论为并行输出模型，与威胁分类（`TrackStateSnapshot::target_type`）
 * 相互独立，不共享字段或权重。
 */

#ifndef ONEQ_AIRBORNE_RADAR_SESSION_AR_RECOGNITION_RESULT_H_
#define ONEQ_AIRBORNE_RADAR_SESSION_AR_RECOGNITION_RESULT_H_
...
/**
 * @brief ArRecognitionState 识别状态机。
 */
...
/**
 * @brief ArRecognitionFeatureDimension 特征维度位掩码。
 */
...
/**
 * @brief ArRecognitionFeatureScores 四类特征的相似度与质量。
...
/**
 * @brief ArRecognitionResult 单条航迹的识别结论。
 */
...
/**
 * @brief ArRecognitionCycleSummary 单周期识别效能摘要（嵌入 ArCycleResult）。
 */
...
#endif  // ONEQ_AIRBORNE_RADAR_SESSION_AR_RECOGNITION_RESULT_H_
