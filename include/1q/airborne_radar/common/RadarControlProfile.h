...
struct RadarControlProfile {
  ...
  std::uint32_t lpi_cooldown_cycles_remaining{0}; /**< LPI 域释放后的冷却周期，仅供 reducer 运行态使用 */
  std::uint32_t eccm_cooldown_cycles_remaining{0}; /**< ECCM 域释放后的冷却周期，仅供 reducer 运行态使用 */
};
