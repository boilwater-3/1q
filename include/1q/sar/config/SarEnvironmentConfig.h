 ...
 *       内部生成 raw echo 还可按目标双程斜距施加大气比损耗，并用表面后向散射
 *       系数生成场景中心附近的确定性分布式地表背景。
 */
struct ONEQ_API SarEnvironmentConfig {
  ...
  double surface_backscatter_sigma0_db{-12.0}; /**< 分布式地表背景 sigma0（dB） */
  bool use_flat_earth_geometry{true};         /**< true 使用局部扁平近似；false 使用 WGS-84 ENU */
  bool enable_atmospheric_attenuation{false}; /**< 是否对内部生成回波施加双程大气衰减 */
};
