...
enum class SarProcessingStage {
  kNone = 0,
  kRawEcho = 1,
  /// 距离压缩阶段标记。当前 Phase 1 不产出独立可消费的距离压缩产物，真实距离压缩在
  /// RDA / BP 内部完成；该值表示会话已声明完成该内部步骤、满足 L3 BP 前置条件门，
  /// 并作为 replay 摘要保真。它不是终端成像阶段，仅当 RDA/BP 未启用时才可能成为
  /// completed_stage 的最终值。
  kRangeCompression = 2,
  kL1RdaImage = 3,
  kL3BpImage = 4
};
...
struct ONEQ_API SarOutputFrame {
  ...
  /// 距离压缩摘要标志，与 `SarProcessingStage::kRangeCompression` 同义。当前 Phase 1
  /// 不返回独立距离压缩载荷；该标志在 `enable_range_compression` 为真时置位，表示
  /// RDA/BP 内部距离压缩步骤已声明完成，并非可消费的独立输出。
