 ...
 *
 * 调用方提供 LLA 平台/目标与外部脉冲运动学列表，即可获得可直接传入
 * SarSession::StepWithResult() 的 SarCycleInput。脉冲坐标转换以
 * mission.scene_center_* 为局部原点。
 *
 * @note 当 external_pulses 为空时，raw_iq 保持默认空值，SarSession
 *       将走内部 raw echo 生成路径（而非外部 IQ 路径）。
 */
