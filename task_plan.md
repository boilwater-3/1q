...
| 70 | CSA 数学与参考真值工程契约 | complete | 冻结频率几何基础；完整相位函数等待独立真值 |
...
| 75 | Omega-K Stolt 几何后续决策门 | complete | 完整 Omega-K 与选择器接入后置；选择自聚焦误差契约 |
| 76 | 自聚焦残余相位误差与参考真值工程契约 | complete | 冻结低阶真值、不可观测分量与校正诊断 |
| 77 | 自聚焦相位误差真值基础实现 | complete_current_platform | 剖面、最小二乘去除、校正真值与双环境验收通过 |
...
| 87 | 时变 PRF 质量矩阵后续决策门 | complete | RDA 接入后置；选择缺失脉冲与最大间隙诊断契约 |
| 88 | 缺失脉冲与最大慢时间间隙诊断契约 | complete | 冻结显式期望间隔、1.5x 门、缺失计数与拒绝语义 |
| 89 | 缺失脉冲与最大慢时间间隙诊断实现 | complete_current_platform | 间隙诊断、门禁重采样与双环境验收通过 |
| 90 | 缺失脉冲诊断后续接入决策门 | complete | 参考链可接入；生产 RDA/Session 后置；选择拒绝矩阵 |
...
2. 冻结内部请求身份、显式路径、指定像素、斜距、RCS 和孔径来源。
3. 冻结无状态执行器、原子失败和不得影响原聚焦结果语义。
4. 冻结 RDA/GBP/BP 首批支持边界与失败原因。
5. public API、SarSession、schema、trace、replay 和有效性阈值继续后置。
...
1. CSA/Omega-K 缺少二维频率口径、插值/相位契约和独立真值，继续后置。
2. 自聚焦缺少残余相位误差模型和独立真值，继续后置。
3. 时变 PRF 几何已具备，但 RDA 仍假设均匀慢时间，继续后置。
4. RDA/GBP/BP 已具备明确证据，下一方向选择内部确定性聚焦算法选择器。
5. 新增 `docs/sar_remaining_focusing_capability_audit.md`。
...
2. 冻结轨迹等级、调用目的、尺寸、算法可用性和补偿/航路点显式输入。
3. 冻结 L1/L2 RDA、独立 GBP 参考和 L3 BP 建议规则。
4. 冻结算法不可用、尺寸超限和输入矛盾时拒绝且不静默切换。
5. public Auto、Session、schema、trace、replay 和 runtime patch 继续后置。
...
1. 在 `src/sar/imaging` 实现内部无状态建议器与结构化原因。
2. 增加 L1/L2/L3、用途、算法可用性和尺寸边界决策矩阵。
3. 验证确定性、输入不变和不调用聚焦算法。
4. 新增 `docs/sar_internal_focusing_selector_acceptance_report.md`。
5. 默认完整 CTest `25/25`、双环境选择器测试各 `4/4`、Eigen 3.3.9 C++11 门通过。
...
2. 冻结未 shift 距离/方位频率轴、`D(f_a)`、`alpha(f_a)` 和有效域。
3. 冻结首批 CSA 频率几何诊断与奇偶长度验收矩阵。
4. 识别现有文档缺少完整相位公式，完整 CSA 等待独立中间域真值。
5. public API、Session、Auto、时变 PRF 和尺寸扩展继续后置。
...
1. 在 `src/sar/imaging` 实现未 shift 频率轴和 CSA 几何诊断。
2. 增加奇偶轴、对称性、零 Doppler、有效域与无效域测试。
3. 与现有 RDA Doppler 诊断交叉核对。
4. 新增 `docs/sar_csa_frequency_geometry_acceptance_report.md`。
5. 默认完整 CTest `25/25`、双环境几何测试各 `4/4`、Eigen 3.3.9 C++11 门通过。
...
1. 仓库不具备完整 CSA 相位函数、固定中间域真值或独立参考脚本。
2. 完整 CSA 聚焦与内部选择器接入继续后置。
3. 几何基础保持独立内部诊断，不扩大 public/Session/schema/replay 表面。
4. 下一方向选择 Omega-K 数学与参考真值工程契约。
5. 新增 `docs/sar_csa_frequency_geometry_followup_decision.md`。

## 阶段 73：Omega-K 数学与参考真值工程契约
...
2. 冻结双程距离波数、方位波数、传播波数与有效域。
3. 冻结均匀目标 `K_z` 反求源距离频率的 Stolt 查询映射。
4. 冻结无效色散点、越支持区查询和最大 shift 诊断。
5. 完整参考相位、复数插值和聚焦入口继续后置。
...
1. 仓库缺少复数 Stolt 插值真值、支持区策略和完整参考相位。
2. 完整 Omega-K 聚焦与内部选择器接入继续后置。
3. Stolt 几何保持独立内部诊断，不扩大 public/Session/schema/replay 表面。
4. 下一方向选择自聚焦残余相位误差与参考真值工程契约。
5. 新增 `docs/sar_omega_k_stolt_geometry_followup_decision.md`。

## 阶段 76：自聚焦残余相位误差与参考真值工程契约
...
2. 冻结归一化孔径坐标与低阶多项式相位误差真值。
3. 冻结常量/线性不可观测分量的离散最小二乘去除。
4. 冻结可观测残余、校正剖面、误差指标和验收矩阵。
5. PGA、图像熵优化、生产图像修改和 Session 接入继续后置。
...
1. 在 `src/sar/imaging` 实现低阶相位误差剖面与不可观测分量拟合。
2. 输出可观测残余、真值校正和结构化诊断。
3. 增加常量/线性退化、二次/三次恢复、非法输入和确定性测试。
4. 新增 `docs/sar_autofocus_phase_truth_acceptance_report.md`。
5. 默认完整 CTest `25/25`、双环境真值测试各 `4/4`、Eigen 3.3.9 C++11 门通过。
...
1. 仓库缺少 PGA 支持区、梯度估计真值、unwrap 和停止准则。
2. PGA、生产图像校正与 Session 接入继续后置。
3. 相位误差真值保持独立内部诊断能力。
4. 下一方向选择时变 PRF 慢时间采样与重采样工程契约。
5. 新增 `docs/sar_autofocus_phase_truth_followup_decision.md`。

## 阶段 79：时变 PRF 慢时间采样与重采样工程契约
...
5. 非均匀生产成像、NUFFT、Session 和 public 接入继续后置。

## 阶段 80：时变 PRF 慢时间重采样基础实现

状态：`complete_current_platform`

已完成：

1. 在 `src/sar/imaging` 实现显式时间轴诊断和名义均匀轴。
2. 实现复数慢时间向量线性重采样。
3. 增加均匀退化、非均匀诊断、仿射真值和非法输入测试。
4. 新增 `docs/sar_slow_time_resampling_acceptance_report.md`。
5. 默认完整 CTest `25/25`、双环境重采样测试各 `4/4`、Eigen 3.3.9 C++11 门通过。
...
2. 冻结 `ComplexMatrix` row=方位、col=距离轴与尺寸边界。
3. 冻结所有列共享同一慢时间查询映射和首尾行保持。
4. 冻结多列仿射解析真值、列独立性和拒绝语义。
5. RDA 接入、带限回波阈值和缺失脉冲策略继续后置。
...
1. 仓库具备参考场景、RDA/GBP 和图像比较指标，可建立质量矩阵。
2. 当前没有抖动幅度、raw-history 误差和聚焦质量通过区证据。
3. RDA、Session、public、schema、trace 和 replay 接入继续后置。
4. 下一方向选择时变 PRF 重采样质量参考矩阵契约。
5. 新增 `docs/sar_raw_history_resampling_followup_decision.md`。

## 阶段 85：时变 PRF 重采样质量参考矩阵契约
...
2. 冻结首尾固定的确定性正弦时间抖动与严格递增约束。
3. 冻结 raw-history NRMS、RDA 图像 NRMS/相干相关和时间轴诊断。
4. 冻结零、小、中、大抖动矩阵与趋势验收。
5. 缺失脉冲、生产 RDA 与 Session/public 接入继续后置。
...
1. 当前单点矩阵不足以冻结通用阈值或批准 RDA 接入。
2. 缺失脉冲、多目标、高 Doppler 和随机抖动仍无证据。
3. RDA、Session、public、schema、trace 和 replay 接入继续后置。
4. 下一方向选择缺失脉冲与最大慢时间间隙诊断契约。
5. 新增 `docs/sar_variable_prf_quality_followup_decision.md`。

## 阶段 88：缺失脉冲与最大慢时间间隙诊断契约
...
2. 冻结显式 expected interval，禁止由首尾跨度反推缺失检测基准。
3. 冻结 `1.5x` 最大间隙门、疑似缺失计数和首个拒绝索引。
4. 冻结允许/拒绝语义与均匀、小抖动、单/多缺失验收矩阵。
5. 缺失修复、生产聚焦和 Session/public 接入继续后置。

## 阶段 89：缺失脉冲与最大慢时间间隙诊断实现

状态：`complete_current_platform`

已完成：

1. 在 `src/sar/imaging` 实现独立间隙诊断。
2. 增加门禁向量与二维慢时间重采样包装入口。
3. 增加均匀、小抖动、边界、单/多缺失和非法输入测试。
4. 新增 `docs/sar_missing_pulse_gap_diagnostics_acceptance_report.md`。
5. 默认完整 CTest `25/25`、双环境诊断测试各 `5/5`、Eigen 3.3.9 C++11 门通过。
...
1. 参考场景 `prf_hz` 可作为明确 expected interval 真值。
2. 内部参考质量链可接入门禁包装入口。
3. Session 缺少显式时变 PRF 请求语义，生产 RDA/Session/public 接入继续后置。
4. 下一方向选择缺失脉冲拒绝参考矩阵契约。
5. 新增 `docs/sar_missing_pulse_gap_followup_decision.md`。
...
2. 冻结 baseline、小抖动、`1.5x` 边界、单缺失和多缺失矩阵。
3. 冻结允许 case 重采样/RDA 完成语义。
...
1. 使用固定参考场景实现允许/拒绝矩阵。
2. 验证缺失计数、首个拒绝索引和停止语义。
3. 验证允许 case 可完成重采样与 RDA。
4. 新增 `docs/sar_missing_pulse_rejection_matrix_acceptance_report.md`。
5. 默认完整 CTest `25/25`、默认定向测试 `3/3`、Eigen 3.3.9 SAR 单元入口和 C++11 门通过。
...
1. Session/public 缺少显式时变 PRF 请求和失败传播契约。
2. RDA 默认路径接入与缺失修复、稀疏恢复、NUFFT 继续后置。
3. 下一方向选择内部慢时间重采样请求与执行边界契约。
4. 新增 `docs/sar_missing_pulse_rejection_followup_decision.md`。
...
4. 新增验收报告并通过默认/Eigen 3.3.9 双环境 SAR 审批门。

## 阶段 96：内部慢时间重采样执行器后续接入决策门

状态：`complete`

已完成：

1. 审计 Session、RDA 与 replay/schema 的现有边界。
2. 确认固定 PRF Session/RDA 与 public/schema/replay 均缺少无歧义接入语义。
3. 生产链接入继续后置，保持内部显式执行器边界。
4. 下一方向选择扩展时变 PRF 重采样质量矩阵契约。

## 阶段 97：扩展时变 PRF 重采样质量矩阵契约

状态：`complete`

已完成：

1. 冻结 M1/M4、中心/高 Doppler 与确定性随机小抖动矩阵。
2. 冻结 raw-history、RDA 图像质量与执行器诊断指标。
3. 冻结默认/Eigen 3.3.9 双环境与 C++11 验收门。
4. 保持缺失脉冲为拒绝 case，并继续后置生产/public/replay 接入。

## 阶段 98：扩展时变 PRF 重采样质量矩阵实现

状态：`complete_current_platform`

已完成：

1. 实现确定性随机慢时间抖动 helper 与 E1-E3/J0-J4 矩阵。
2. 通过内部执行器重采样，并记录 raw-history/RDA 图像质量。
3. 验证确定性、严格退化、间隙门和双环境审批门。
4. 新增验收报告并记录测量结果。

## 阶段 99：扩展时变 PRF 质量矩阵后续决策门

状态：`complete`

已完成：

1. 确认小型 L1 扩展矩阵不足以批准生产 RDA/Session 接入。
2. 冻结时变 PRF 内部闭环的当前证据边界与禁止项。
3. 下一方向选择 CSA 完整相位函数与中间域参考真值工程契约。

## 阶段 100：CSA 完整相位函数与中间域参考真值工程契约

状态：`complete`

已完成：

1. 确认仓库现有材料不足以无歧义编码完整 CSA 相位公式。
2. 冻结三类相位核、显式操作顺序与逐阶段固定复数真值参考包。
3. 冻结中间域允许操作、有效域、失败语义、首批小矩阵与禁止项。
4. 为未来完整公式与执行链建立逐阶段审批入口。

## 阶段 101：CSA 中间域真值执行器实现

状态：`complete_current_platform`

已完成：

1. 实现内部显式操作列表与逐阶段真值执行器。
2. 支持行/列 FFT、IFFT 和同形状单位模相位核相乘。
3. 增加冲激、复指数、非对称矩阵与结构化拒绝测试。
4. 完成双环境与 C++11 验收，不新增完整 CSA 聚焦入口。

## 阶段 102：CSA 中间域真值执行器后续决策门

状态：`complete`

已完成：

1. 确认仓库仍缺少经审计完整 CSA 公式、真实相位核参考包与逐阶段真值。
2. 完整 CSA 聚焦、选择器和生产接入继续后置。
3. 中间域真值执行器保留为未来公式参考包的强制审批门。
4. 下一方向选择 Omega-K 复数 Stolt 插值与中间域参考真值契约。
