...
| 67 | 内部确定性聚焦选择器契约 | pending | 冻结输入、建议、原因与拒绝语义 |
...
1. 冻结选择器显式输入、算法可用性、场景尺寸和调用目的。
2. 冻结 RDA/GBP/BP 建议规则、原因和拒绝语义。
3. 冻结选择器只建议、不执行、不回退的边界。
4. public Auto、schema、trace、replay 和 runtime patch 继续后置。
