...
| 60 | 显式标定观测工程契约 | pending | 冻结观测来源、功率提取与生命周期 |
...
1. 冻结显式观测对象、标定目标身份和斜距来源。
2. 冻结未归一化峰值功率提取和像素定位口径。
3. 冻结定标结果生命周期、替换和失效边界。
4. public API、Session schema、replay 和有效性阈值继续后置。
