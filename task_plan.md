...
| 90 | 缺失脉冲诊断后续接入决策门 | pending | 审计 expected interval 来源与调用链接入条件 |
...
1. 审计 expected interval 在内部调用链中的显式可信来源。
2. 决定是否接入现有二维重采样参考链。
3. 确认 RDA、Session、public/schema/replay 是否继续后置。
