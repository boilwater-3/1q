...
| 79 | 时变 PRF 慢时间采样与重采样工程契约 | pending | 冻结时间轴、非均匀诊断、重采样与真值要求 |
...
1. 冻结显式脉冲时刻、名义均匀时间轴与非均匀度诊断。
2. 冻结首批重采样支持区、插值边界和拒绝语义。
3. 定义独立解析信号真值与验收矩阵。
4. 明确首批可实现基础与非均匀成像后置边界。
...
执行阶段 79：冻结时变 PRF 慢时间采样与重采样工程契约。
