...
| 76 | 自聚焦残余相位误差与参考真值工程契约 | pending | 冻结误差模型、不可观测分量、支持区与真值要求 |
...
1. 冻结首批可观测残余相位误差模型与不可观测分量。
2. 冻结 PGA 输入支持区、估计输出与拒绝语义。
3. 定义独立注入真值、误差指标和验收矩阵。
4. 明确首批可实现基础与通用自聚焦后置边界。
